...
(function () {
  ...
  function ensureButton() {
    ...
    shadow.innerHTML = `
      <style>
        ...
        button:hover { background: #1c1c1f; }
        button[disabled] { opacity: .55; cursor: wait; }
        .hint { color: #a1a1aa; font-size: 11px; padding-right: 4px; }
      </style>
      <div class="wrap">
        <button type="button" id="go" title="Haul out this conversation (Alt+Shift+E)">Haul out</button>
        <div class="hint">Alt+Shift+E</div>
      </div>
    `;
    const go = shadow.getElementById("go");
    go.addEventListener("click", () => startExport(go));
  ...
  document.addEventListener("keydown", (e) => {
    if (e.altKey && e.shiftKey && (e.key === "E" || e.key === "e") && !e.repeat) {
      e.preventDefault();
      const fab = document.getElementById(BTN_ID);
      const btn = fab && fab.shadowRoot && fab.shadowRoot.getElementById("go");
      startExport(btn);
    }
  }, true);
...
async function haulOut() {
  ...
  try {
    ...
    try {
      ...
      console.warn("[HaulOut] API path failed:", err);
    }
    ...
    const base = `haulout-${stamp.slice(0, 10)}-${platform}-${slug}`;
  ...
  /* ------------------------------------------------------------------ */
  function makeUI() {
    const existing = document.getElementById("haulout-ui");
    ...
    box.innerHTML = `
      <div style="font-weight:650;margin-bottom:6px">Haul out</div>
      <div id="haulout-status" style="color:#a1a1aa;margin-bottom:10px">…</div>
      <div id="haulout-actions" style="display:flex;gap:8px;flex-wrap:wrap"></div>
    `;
    document.documentElement.appendChild(box);
    const statusEl = box.querySelector("#haulout-status");
    const actions = box.querySelector("#haulout-actions");
    return {
      status(t) { statusEl.textContent = t; },
      chooseFormat() {
        return new Promise((resolve) => {
          statusEl.textContent = "Haul out this conversation as:";
          actions.innerHTML = "";
